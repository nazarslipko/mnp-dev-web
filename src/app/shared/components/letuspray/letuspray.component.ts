...
@Component({
  ...
  templateUrl: './letuspray.component.html',
  styleUrl: './letuspray.component.css',
})
export class LetusprayComponent implements OnInit {
  iContact: IContact;
  ...
  async ngOnInit() {   
    this.activatedRoute.paramMap.subscribe((params) => {   
     this.categoryLookupId = this.getCategoryNumber(params.get('category'));
    });

    this.sortScrollByAscDesc = 0;
    this.scrollRequest = 'scroll';
    await this.getLookupByCategory('scroll');
    this.buildScrollForm();
    this.getScrollData();
  }

  getCategoryNumber(category: any): number {
      ...
      .pipe(
        switchMap((lookups: ILookup[]) => {
          if (category === 'scroll') {
            ...
            return of(this.scrollCategory);
          } else {
            this.requestServiceCategory = lookups;
            return of(this.requestServiceCategory);
          }
        })
      )
      .subscribe({
        next: (lookups: ILookup[]) => {
          console.log('Retrieved Lookups for the scroll and requests');
        },
        error: (err) =>
          console.error(
            'Error retrieving Lookups for the scroll and requests'
          ),
        complete: () => console.log('Complete retrieving Lookups for the scroll and requests')
      });
  }

  // build the scroll form
  buildScrollForm() {
    const tagPattern = '^#[a-zA-Z0-9]+(,#[a-zA-Z0-9]+)*$'; // '^[0-9a-zA-Z]+(,[0-9a-zA-Z]+)*$';
  ...
  postOnScroll() {
    ...
    data.AffiliationLookupId = this.iContact.AffiliationLookupId;
    data.UserId = !Number.isNaN(this.iContact.UserId) ? this.iContact.UserId : 0;
    ...
    this.scrollService
      ...
      .subscribe({
        next: (theScroll) => {
          console.log('Save message on the scroll');
        },
        error: (err: any) => {
  ...
  // save user reactions
  scrollStoryReactions(scroll: any, reaction: any, signInModal: any) {
    ...
    if (typeof this.iContact?.UserId !== 'undefined' || Number.isNaN(this.iContact?.UserId)) {
      ...
      reactionData.UserId = this.iContact.UserId;
      ...
      this.reactionService
        ...
        .subscribe({
          next: (iReaction: IReaction) => {
            console.log('Save scroll reaction')
          },
          error: (err: any) => {
            this.errors = err.error.Message;
          },
  ...
  async getCountryTimezone(countryCode: any) {
    ...
    this.timeZoneService
      ...
      .subscribe({
        next: (iTimezone) => {
          console.log("Retrieved TimeZones");
        },
        error: (err) => console.log(' Error retrieving timezones')
      });
  }
  ...
  startPrayMeeting(lookupId: number, meetingModal: any) {
    //  await this.getLookupByCategory('prayerrequest');
